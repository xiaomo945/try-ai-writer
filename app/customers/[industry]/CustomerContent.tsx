...
const INDUSTRIES: Record<string, Industry> = {
  ...
  travel: {
    ...
    name: "旅游酒店",
    icon: Plane,
    tagline: "帮 40+ 旅游品牌把营销内容效率提升 6x",
    description: "旅游行业需要大量目的地介绍、酒店推荐、攻略内容。Try AI Writer 帮助旅游品牌把内容生产提速,SEO 流量提升。",
    color: "sky",
    customers: 70,
    caseCount: 18,
    avgRoi: "290%",
    ...
    <main className="min-h-screen bg-obsidian-950 text-slate-100">
      ...
      <section className="relative overflow-hidden border-b border-slate-800/60 bg-gradient-to-b from-emerald-950/40 via-obsidian-950 to-obsidian-950">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
          <nav className="flex items-center gap-2 text-sm text-slate-400 mb-8" aria-label="面包屑">
            ...
            <Link href="/customers" className="hover:text-emerald-400">客户案例</Link>
            <ChevronRight className="w-4 h-4" />
          ...
          <div className="grid lg:grid-cols-3 gap-12 items-start">
            <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="rounded-full bg-slate-800 text-slate-300 px-3 py-1 text-xs">
                  {ind.customers.toLocaleString()}+ 企业客户
                </span>
                <span className="rounded-full bg-amber-500/10 text-amber-300 px-3 py-1 text-xs">
                  平均 ROI {ind.avgRoi}
                </span>
              </div>
            ...
            <aside className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-6 space-y-4">
              <div className="text-center">
                <div className="text-3xl font-extrabold text-emerald-400">{ind.avgRoi}</div>
                <div className="text-xs text-slate-400 mt-1">平均投资回报率</div>
              </div>
              <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-800/80">
                <div>
                  <div className="text-xl font-bold text-white">{ind.customers.toLocaleString()}</div>
                  <div className="text-xs text-slate-400 mt-1">企业客户</div>
                </div>
                <div>
                  <div className="text-xl font-bold text-white">{ind.caseCount}</div>
                  <div className="text-xs text-slate-400 mt-1">公开案例</div>
                </div>
              </div>
            </aside>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="rounded-2xl border border-slate-800/80 bg-slate-900/40 p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-serif text-2xl font-extrabold text-white">行业挑战</h2>
            </div>
            <ul className="space-y-2">
              {ind.challenges.map((c, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-slate-200">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-rose-500/15 text-rose-300 text-xs font-bold flex items-center justify-center mt-0.5">!</span>
                  {c}
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-950/20 p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-serif text-2xl font-extrabold text-white">Try AI Writer 方案</h2>
            </div>
            <ul className="space-y-2">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 border-t border-slate-800/60">
        <div className="flex items-center gap-2 mb-8">
          ...
          <h2 className="font-serif text-3xl font-extrabold text-white">量化成果</h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {ind.metrics.map((m, i) => (
            ...
            >
              <div className="text-xs text-slate-400 uppercase tracking-wider mb-2">{m.label}</div>
              <div className="flex items-baseline gap-2 mb-2">
                <div className="text-sm text-slate-500 line-through">{m.before}</div>
                <ArrowRight className="w-3 h-3 text-slate-600" />
                <div className="text-xl font-extrabold text-emerald-300">{m.after}</div>
              </div>
              <div className="text-xs font-semibold text-emerald-400">{m.change}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 border-t border-slate-800/60">
        <div className="flex items-center gap-2 mb-8">
          ...
          <h2 className="font-serif text-3xl font-extrabold text-white">客户案例</h2>
        </div>
            ...
            <div
              ...
              className={`group cursor-pointer rounded-2xl border p-6 transition-all duration-300 ${
                activeCase === i
                  ...
                  : "border-slate-800/80 bg-slate-900/40 hover:border-emerald-500/30"
              }`}
            >
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center text-white font-extrabold text-2xl flex-shrink-0">
                  {cs.logo}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <h3 className="text-lg font-bold text-white">{cs.company}</h3>
                    <span className="rounded-full bg-slate-800 text-slate-300 px-2 py-0.5 text-xs">{cs.size}</span>
                    <span className="rounded-full bg-slate-800 text-slate-300 px-2 py-0.5 text-xs flex items-center gap-1">
                      <Calendar className="w-3 h-3" />
                      {cs.duration}
                    </span>
                  </div>
                  <div className="text-sm text-slate-300 mb-2">
                    <span className="text-emerald-300 font-semibold">使用场景:</span> {cs.useCase}
                  </div>
                  <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-2.5 text-sm text-emerald-200 mb-3">
                    <span className="font-semibold">成果:</span> {cs.result}
                  </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 border-t border-slate-800/60">
        <div className="flex items-center gap-2 mb-8">
          ...
          <h2 className="font-serif text-3xl font-extrabold text-white">{ind.name}行业落地流程</h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {ind.workflow.map((w) => (
            <div
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-serif text-lg font-bold text-white">{w.title}</h3>
              </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 border-t border-slate-800/60">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="font-serif text-2xl font-extrabold text-white">{ind.name}客户常用工具</h2>
        </div>
        <div className="flex flex-wrap gap-2">
          {ind.tools.map((t, i) => (
            <span
              key={i}
              className="rounded-full border border-slate-700 bg-slate-900/60 px-4 py-2 text-sm text-slate-200 hover:border-emerald-500/60 hover:text-emerald-300 transition-all duration-300"
            >
              {t}
            </span>
          ))}
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 border-t border-slate-800/60">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="font-serif text-2xl font-extrabold text-white">其它行业</h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {SLUGS.filter((s) => s !== slugKey).map((s) => {
            const other = INDUSTRIES[s];
            ...
            return (
              ...
              >
                <OIcon className="w-6 h-6 text-emerald-400 mb-2" />
                <div className="text-sm font-semibold text-white group-hover:text-emerald-300">
                  {other.name}
                </div>
                <div className="text-xs text-slate-500 mt-1 line-clamp-2">{other.tagline}</div>
              </Link>
            );
          })}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 border-t border-slate-800/60">
        <div className="rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-950/60 to-teal-950/40 p-8 lg:p-12 text-center space-y-6">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl font-extrabold text-white">
            加入 {ind.customers.toLocaleString()}+ {ind.name}企业的选择
          </h2>
          <p className="text-slate-300 max-w-2xl mx-auto">
            7 天免费试用,5 分钟上手,首月 $5 即可解锁全部功能。
          </p>
          <Link
            href="/signup"
